	...
	if password == "" && cfg.PasswordFile != "" {
		...
		if info, err := os.Stat(pwFile); err == nil {
			...
			if mode&0077 != 0 {
				...
				if data, readErr := os.ReadFile(pwFile); readErr == nil {
					password = strings.TrimSpace(string(data))
					logger.Debug("SSH session %q: password loaded from file %s", cfg.Name, pwFile)
				} else {
					logger.Warn("SSH session %q: failed to read password file %s: %v", cfg.Name, pwFile, readErr)
				}
			}
		} else {
...
func (s *SSHSession) Reconnect() error {
	...
	if savedCwd != "" && savedCwd != "~" {
		// Don't use Execute() as it would run the command
		// Just set the cwd directly and verify it exists
		result, cdErr := s.executeRaw(fmt.Sprintf("cd %s && pwd", savedCwd))
		if cdErr == nil && result.ExitCode == 0 {
			s.cwd = strings.TrimSpace(result.Stdout)
			logger.Debug("SSH restored cwd to %s", s.cwd)
		} else {
			logger.Debug("SSH could not restore cwd %s: %v", savedCwd, cdErr)
		}
	}
...
func (s *SSHSession) ExecuteInteractive(cmdStr string) (int, error) {
	...
	// Request agent forwarding if enabled
	if s.agentForwarding {
		if agentErr := agent.RequestAgentForwarding(session); agentErr != nil {
			logger.Debug("SSH agent forwarding request failed: %v", agentErr)
		}
	}
	...
	if ptyErr := session.RequestPty(termType, height, width, modes); ptyErr != nil {
		return 1, fmt.Errorf("failed to request PTY: %w", ptyErr)
	}
	...
	go func() {
		for range sigwinchCh {
			if w, h, resizeErr := term.GetSize(fd); resizeErr == nil {
	...
	if startErr := session.Start(fullCmd); startErr != nil {
		return 1, fmt.Errorf("failed to start command: %w", startErr)
	}
	...
	go func() {
		...
		for {
			...
			if pollFds[0].Revents&unix.POLLIN != 0 {
				...
				if _, writeErr := stdinPipe.Write(buf[:nr]); writeErr != nil {
					return
				}
			}
