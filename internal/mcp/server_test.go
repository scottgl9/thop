...
func TestMCPServer_Initialize(t *testing.T) {
	...
	if unmarshalErr := json.Unmarshal(responseData[:len(responseData)-1], &response); unmarshalErr != nil { // Remove trailing newline
...
func TestMCPServer_JSONRPCParsing(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			input:   `{"jsonrpc":"2.0","method":"cancelled"}`, // nolint:misspell // MCP protocol standard
			wantErr: false,
		},
	}
...
func TestMCPServer_Stop(t *testing.T) {
	...
	case <-srv.ctx.Done():
	default:
		t.Error("Context should be canceled") // nolint:misspell
	}
}
